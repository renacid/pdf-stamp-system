...
import { DEFAULTS, KEYS, validateSettings, parseSettings, settingsUrl, exceedsPage } from './settings.js';
import { drawStamp, stampPdf, createSamplePdf } from './pdf.js';
...
function updateUrl(s) {
  const url = settingsUrl(s, location.href);
...
async function processLatest() {
  ...
  try {
    const s = readForm();
    if (s.page > originalDoc.numPages) throw new Error(`このPDFは${originalDoc.numPages}ページです。ページ番号を変更してください。`);
    const page = await originalDoc.getPage(s.page);
    const natural = page.getViewport({ scale: 1 });
    await document.fonts.ready;
    const stamp = drawStamp(document.createElement('canvas'), s.text);
    const bytes = await stampPdf(originalBytes, stamp.toDataURL('image/png'), s, natural);
    if (current !== revision) return;
    renderedDoc = await pdfjs.getDocument({ ...pdfOptions, data: bytes.slice() }).promise;
    const stampedPage = await renderedDoc.getPage(s.page);
    ...
    $('page-info').textContent = `${s.page} / ${originalDoc.numPages} ページ`;
    $('dimensions').textContent = `${(natural.width * 25.4 / 72).toFixed(1)} × ${(natural.height * 25.4 / 72).toFixed(1)} mm`;
    outputBytes = bytes;
    $('download').disabled = false;
    if (exceedsPage(s, natural)) status('ハンコが用紙からはみ出しています。用紙の外側は保存時にも表示されません。位置・サイズを確認してください。', 'warning');
    else if (s.opacity === 0) status('不透明度が0のため、ハンコは見えません。設定を確認してください。', 'warning');
    else status('捺印プレビューを更新しました。この仕上がりでPDFを保存できます。');
  } catch (error) {
...
$('reset').addEventListener('click', () => { setForm(DEFAULTS); schedule(); });
...
function restoreUrl() {
  try { setForm(parseSettings(location.search)); schedule(); }
  catch (error) {
    setForm(DEFAULTS); invalidate(); $('settings-url').value = ''; $('copy-url').disabled = true;
...
if (document.modelContext?.registerTool) {
  ...
  try {
    Promise.resolve(document.modelContext.registerTool({
      ...
      execute(input) {
        const s = validateSettings({ ...readForm(), ...input });
        setForm(s); schedule();
